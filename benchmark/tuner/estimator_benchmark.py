"""Benchmark to compare the estimation and actual execution results"""
...
def get_dist_config_str(mp, pp, dp):  # pylint: disable=invalid-name
    """Get configuration string related to distributed training"""
...
def get_model_config_str(num_layers, hidden_size, num_attention_heads, seq_length, fp16):
    """Get configuration string for model specification"""
...
def get_345m_model(fp16, num_layers):
    """Get configuration string for 345 million parameter model"""
...
def get_7b_model(fp16, num_layers):
    """Get configuration string for 7.5 billion parameter model"""
    return get_model_config_str(num_layers=num_layers,
                                hidden_size=4096,
...
def get_39b_model(fp16, num_layers):
    """Get configuration string for 39 billion parameter model"""
    return get_model_config_str(num_layers=num_layers,
                                hidden_size=8192,
...
def get_mock_argv(data_dir, model_config_str):
    """Get arguments to mock"""
...
def get_actual_iter_time_and_memory(data_dir, model_config_str, dist_config_str,  # pylint: disable=too-many-arguments, too-many-locals
    ...
    subproc = subprocess.Popen([full_cmd],  # pylint: disable=consider-using-with, subprocess-popen-preexec-fn
                               shell=True,
                               stdout=subprocess.PIPE if node_rank == 0 else subprocess.STDOUT,
    ...
    while subproc.poll() is None:
        ...
        reg_exp = r"iteration[ \t]+(\d+).*?elapsed time per iteration \(ms\):[ \t]+(\d+\.\d*)"
        reg_exp += r".*?number of skipped iterations: +(\d+)"
        m = re.search(reg_exp, output)  # pylint: disable=invalid-name
        if m:
            iteration = int(m[1])
            iter_time = float(m[2])
            ...
            m = re.search(r"max allocated:[ \t]+(\d+\.\d*)?", output)  # pylint: disable=invalid-name
            if m:
                print(output)
                mem_alloc = float(m[1])
...
def get_estimated_iter_time_and_memory(data_dir, model, world_size, configs_to_test):  # pylint: disable=redefined-outer-name
    """Execute estimator"""
    num_gpus_per_node = int(os.environ['NUM_GPUS_PER_NODE'])
    node_rank = int(os.environ['NODE_RANK'])
    mock_argv = get_mock_argv(data_dir, model)
    ...
    if node_rank == 0:
        for config in configs_to_test:
            ...
            estimated_iter_time, _, _, _ = estimator.get_iter_time(config)
...
def run_estimator_benchmark(data_dir, model, configs_to_test):  # pylint: disable=redefined-outer-name, too-many-locals
    """Execute benchmark for given model and configurations"""
    estimation_results = {}
    ...
    actual_results = []
    for config in configs_to_test:
        os.environ['WORLD_SIZE'] = str(config.mp * config.pp * config.dp)
    ...
    for config, actual in zip(configs_to_test, actual_results):
        ...
        benchmark_logger.info("gpu memory :"
