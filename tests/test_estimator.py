...
from estimator import Config, Estimator
...
def get_actual_iter_time(model_config_str, dist_config_str, distributed_args, global_batch_size, micro_batch_size):
    host_list = os.environ.get('HOST_LIST', 'localhost').split(',')
    subprocs = []
        ...
        with Estimator(world_size, model='gpt') as estimator:
            for mb in [1, 2, 4]:
                for gb_increase in [1, 2, 4]:
                    ...
                    actual_iter_time = get_actual_iter_time(model_config_str,
                                                            dist_config_str,
                                                            distributed_args,
